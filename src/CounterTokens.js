var CounterTokens =
  ...
  (function () {
    ...
    const checkInstall = () => {
      ...
      log(
        '-=>Counter Tokens schema version: [' +
          state.CounterTokens.schemaVersion +
          ']<=-'
      )
      ...
      _.each(persistentTokens, (persistentToken) => {
        if (debug) {
          log(
            'Reattaching token: ' +
              persistentToken.tokenName +
              ' to counter: ' +
              persistentToken.counterName
          )
        }
    ...
    const getImgSrcByTokenName = (tokenObjName) => {
      ...
      if (img.length <= 0) {
        throw new Error(
          'Cannot find token on current page named ' + tokenObjName
        )
      }
    ...
    const attachTokenToCounter = (persistentToken, pageId) => {
      ...
      if (!(pageId in persistentToken.pages)) {
        persistentToken.pages = {
          imgIds: [],
          startY: persistentToken.top,
          lastY: persistentToken.top,
        }
      }
    ...
    const detachTokenFromCounter = (token, pageId) => {
      ...
      if (
        'onClearImages' in tokenForCallback &&
        _.isFunction(tokenForCallback.onClearImages)
      ) {
    ...
    const onPageFirstPlayer = (pageId) => {
      ...
      _.each(persistentTokens, (persistentToken) =>
        attachTokenToCounter(persistentToken, pageId)
      )
    }
    ...
    const removeToken = (tokenName) => {
      ...
      const persistentTokenFound = _.some(
        persistentTokens,
        (persistentToken) => persistentToken.tokenName === tokenName
      )
      const tokenFound = _.some(
        tokens,
      ...
      if (!persistentTokenFound) {
        if (debug) {
          log(
            'Token not found in persistent tokens, can only attempt to delete in-memory token references.'
          )
        }
        if (tokenFound) {
          if (debug) {
    ...
    const isInRange = (
      ...
      const xCoordsConflict =
        (tokenStartX >= minX && tokenStartX <= maxX) ||
        (tokenLastX >= minX && tokenLastX <= maxX)
      const yCoordsConflict =
        (tokenStartY >= minY && tokenStartY <= maxY) ||
        (tokenLastY >= minY && tokenLastY <= maxY)
      ...
      if (yCoordsConflict && xCoordsConflict) {
        if (debug) {
          ...
          log(
            'Minimum X range is: >' + minX + ' and maximum range is: <' + maxX
          )
          log(
            'Minimum Y range is: >' + minY + ' and maximum range is: <' + maxY
          )
        }
        return true
      }
    ...
    const onImageRearrange = (tokenName, pageId, graphicRefs) => {
      if (debug) {
        log(
          'Rearranging images for token: ' + tokenName + ' on page: ' + pageId
        )
        log('Graphic refs: ')
        log(graphicRefs)
      }
      const { newStartX, newLastX, newStartY, newLastY } = getNewBoundingBox(
        ...
        _.max(graphicRefs, (graphicRef) => graphicRef.get('left')).get(
          'left'
        ) || 0,
      ...
      const { oldStartX, oldLastX, oldStartY, oldLastY } = updateCoords(
        tokenName,
        pageId,
        newStartX,
        newLastX,
    ...
    const getNewBoundingBox = (
      ...
      if (debug) {
        log(
          `Getting new bounding box for token: ${tokenName} with graphicMinLeft ${graphicMinLeft}, graphicMaxLeft: ${graphicMaxLeft}, graphicMinTop: ${graphicMinTop} and graphicMaxTop: ${graphicMaxTop}`
        )
      }
    ...
    const updateCoords = (
      ...
      if (
        !(tokenName in persistentTokens) ||
        !(pageId in persistentTokens[tokenName].pages)
      ) {
      ...
      if (debug) {
        log(
          'Updating start and last Y positions for token: ' +
            tokenName +
            ' on page: ' +
            pageId +
            ' to ' +
        ...
        log(
          'Updating start and last X positions for token: ' +
            tokenName +
            ' on page: ' +
            pageId +
            ' to ' +
    ...
    const removeCollisions = (
      ...
      if (debug) {
        log(
          'Removing any collisions with token: ' +
            tokenName +
            ' on page: ' +
            pageId
        )
      ...
      if (
        ...
        if (debug) {
          log(
            `No change in startY or lastY (${startY} == ${changedPageToken.startY} && ${lastY} == ${changedPageToken.lastY}).`
          )
          log(
            `No change in startX or lastX (${startX} == ${changedPageToken.startX} && ${lastX} == ${changedPageToken.lastX}).`
          )
          log('Skipping.')
        }
        return
      ...
      const collidingToken = _.chain(persistentTokens)
        ...
        .find((persistentToken) => {
          if (debug) {
            log(
              'About to check the token for collision: ' +
                persistentToken.tokenName
            )
          ...
          const pageToken = _(persistentToken.pages || {})
            ...
            ) // Check if the token collides with
          if (pageToken && debug) {
            log('Token collides')
            log(pageToken)
          } else if (debug) {
      ...
      changedPageToken.startY =
        collidingPageToken.lastY +
        Math.max(changedToken.height, collidingToken.height) +
        Math.max(collidingToken.spaceY, changedToken.spaceY)
    ...
    const logToChat = (target, msg) => {
      sendChat('API:CounterTokens', '/w ' + target + ' ' + msg, null, {
        noarchive: true,
      })
    }
        ...
        switch (cmd) {
          ...
          case '!counter-tokens.add':
            try {
              ...
              logToChat(
                msg.who,
                'Unable to add new Counter Token: ' + err.message
              )
              logToChat(
                msg.who,
                'Did you mean to use !counter-tokens.add-by-name'
              )
            }
            break
          ...
          case '!counter-tokens.remove':
            try {
              ...
              logToChat(
                msg.who,
                'Unable to remove Counter Token: ' + err.message
              )
            }
            break
          ...
          default:
            logToChat(
              msg.who,
              cmd +
                ' is not a valid CounterTokens command.  Please use !counter-tokens.help'
            )
            break
        }
      } catch (err) {
    ...
    const CounterToken = (function () {
      ...
      const getGraphicRefsBefore = (graphicRefs, current) => {
        ...
        const graphicsBefore = _.filter(
          graphicRefs,
          (g) => getIndexFromName(g.get('name')) < index
        )
      ...
      const findStartLeft = (
        ...
        const left =
          rowsOfFive > 0
            ? ((numGraphicsBefore - rowsOfTen * 10 - rowsOfFive * 5) % 5) *
              spaceRight
            : ((numGraphicsBefore - rowsOfTen * 10) % 10) * spaceRight
        return currentLeft - left
      }

      const findStartLeftAndTop = (
        graphicRefs,
        current,
        spaceRight,
        spaceBottom
        ...
        if (debug) {
          log(
            'Scanning ' + graphicRefsBefore.length + ' graphics before current.'
          )
        }
        ...
        if (graphicRefsBefore.length <= 0) {
          if (debug) {
            log(
              'No graphics before, returning current left and top: ' +
                currentLeft +
                ', ' +
                currentTop
            )
        ...
        const rowsOfFive =
          remainingGraphicsAfterTens < 10
            ? Math.floor((graphicRefsBefore.length - rowsOfTen * 10) / 5)
            : 0
        const remainingGraphicsAfter5s =
          remainingGraphicsAfterTens - rowsOfFive * 5
        const rowsOfLtFive =
          remainingGraphicsAfter5s < 5
            ? graphicRefsBefore.length - rowsOfTen * 10 - rowsOfFive * 5 > 0
              ? 1
              : 0
            : 0
        const top =
        ...
        const left = findStartLeft(
          graphicRefsBefore.length,
          rowsOfTen,
          rowsOfFive,
          currentLeft,
          spaceRight
      ...
      const getNewLeftAndTop = (
        ...
        const [left, top] =
          numOnRow >= 10 || (numOnRow >= 5 && numOnRow + remainingItems < 10)
            ? [startLeft, currentTop + spaceBottom]
            : [currentLeft + spaceRight, currentTop]
      ...
      const imageArranger = (
        ...
        _.reduce(
          ...
          },
          {
            left: startLeft,
            top: startTop,
            remainingItems: graphicRefs.length,
            numOnRow: 0,
          }
        )

      const rearrangeImages = (graphicRefs, startLeft, startTop, token) => {
        const [spaceRight, spaceBottom] = [
          token.spaceX + token.width,
          token.spaceY + token.height,
        ]
        ...
        if (debug) {
          log(
            'Rearranging images  with startLeft: ' +
              startLeft +
              ', startTop: ' +
              startTop +
              ', spaceRight: ' +
      ...
      const undoDeleteIfGraphicShouldExist = (
        graphicRefs,
        token,
        pageId,
        obj
        ...
        if (debug) {
          log(
            'Undoing delete of graphic should exist for token: ' +
              token.tokenName +
              ' on page: ' +
              pageId
          )
        ...
        if (!tokenNameMatches(token.tokenName, objName)) {
          if (debug) {
            log(
              `Graphic name ${objName} doesn't match token ${token.tokenName}, skipping.`
            )
          }
          return
        }
      ...
      const keepImagesTogether = (graphicRefs, token, obj, prev) => {
        if (
          ...
          if (debug) {
            log(
              "No change in position or graphic name doesn't match token, skipping."
            )
          }
          return
        }
        ...
        const [left, top] = findStartLeftAndTop(
          graphicRefs,
          obj,
          token.spaceX + token.width,
          token.spaceY + token.height
      ...
      const addTokenImages = (graphicRefs, token, pageId, numberToAdd) => {
        ...
        if (debug) {
          log(
            `Adding ${numberToAdd} images to token ${token.tokenName} on page ${pageId} starting at ${startValue}`
          )
        }

        _.each(_.range(startValue, startValue + numberToAdd), function (i) {
          const existingImages = findObjs({
            type: 'graphic',
          ...
          if (debug) {
            if (existingImage) {
              ...
              log(
                'No existing image found for index ' +
                  i +
                  ', creating new image.'
              )
      ...
      const updateImages = (graphicRefs, token, pageId, counterValue) => {
        if (debug) {
          log(
            `Updating images for token ${token.tokenName} on page ${pageId} with counter value ${counterValue}`
          )
          log('Graphic refs: ')
          log(graphicRefs)
        ...
        if (counterValue < graphicCount) {
          ...
          removeTokenImages(
            ...
            token,
            pageId,
            token.pages[pageId].imgIds.length - counterValue
          )
        } else if (counterValue > graphicCount) {
          if (debug) {
            log(
              'Adding ' +
                (counterValue - graphicCount) +
                ' images to token: ' +
                token.tokenName
            )
          }
          addTokenImages(
            ...
            token,
            pageId,
            counterValue - token.pages[pageId].imgIds.length
          )
        } else {
          if (debug) {
        ...
        rearrangeImages(
          graphicRefs,
          graphicRefs[0].get('left'),
          graphicRefs[0].get('top'),
          token
        )
      }
      ...
      const getExistingGraphicReferences = (persistentToken, pageId) => {
        ...
        _.each(idsToRemove, (imgId) =>
          tokenPageImageIds.splice(_.indexOf(tokenPageImageIds, imgId))
        )

        return _.sortBy(graphics, (g) => getIndexFromName(g.get('name')))
      }
      ...
      const onCounterChange = (
        ...
        try {
          ...
          logToChat(
            'gm',
            `Unable to update Counter Token images for token: ${token.tokenName}, pageId: ${pageId}, error: ${err.message}`
          )
        }
      }
      ...
      const create = (token, pageId) => {
        ...
        const graphicsMoveHandler = _.partial(
          keepImagesTogether,
          graphicRefs,
          token
        )
        const counterChangeHandler = _.partial(
          onCounterChange,
          graphicRefs,
          token,
          pageId
        )
        const clearImageHandler = _.partial(
          clearImages,
          graphicRefs,
          token.tokenName,
          pageId
        )
        const onCollision = _.partial(
          resetYPosition,
          graphicRefs,
          token,
          pageId
        ...
        const deleteHandler = _.partial(
          undoDeleteIfGraphicShouldExist,
          graphicRefs,
          token,
          pageId
        )
        ...
        on('destroy:graphic', (obj) => {
          callWithMutex(() => deleteHandler(obj))
        })
        on('change:graphic:left', (obj, prev) => {
          callWithMutex(() => graphicsMoveHandler(obj, prev))
        })
        on('change:graphic:top', (obj, prev) => {
          callWithMutex(() => graphicsMoveHandler(obj, prev))
        })
        on('change:graphic:width', (obj) => {
          callWithMutex(() => resizeImageHandler(obj))
        })
        on('change:graphic:height', (obj) => {
          callWithMutex(() => resizeImageHandler(obj))
        })
...
on('ready', function () {
  ...
  const pageWatchIntId = setInterval(() => {
    if (undefined !== PageWatcher) {
      ...
      clearInterval(pageWatchIntId)
      throw new Error(
        'Unable to find state.PageWatcher.ObservePageAddPlayer, have you installed PageWatcher?'
      )
    }
  }, 200)
